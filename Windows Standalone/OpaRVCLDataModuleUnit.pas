...
type
  TOpaRVCLDataModule = class(TDataModule)
...
var
  OpaRVCLDataModule: TOpaRVCLDataModule;
...
procedure TOpaRVCLDataModule.DataModuleDestroy(Sender: TObject);
begin
  FreeAndNil(FInputData);
end;

procedure TOpaRVCLDataModule.DataModuleCreate(Sender: TObject);
begin
  try
...
procedure TOpaRVCLDataModule.InputDataResourceReceived(const Sender: TObject;
  const AResource: TRemoteResource);
begin
...
procedure TOpaRVCLDataModule.acRunClusterExecute(Sender: TObject);
var
...
procedure TOpaRVCLDataModule.acNewParamExecute(Sender: TObject);
var
...
procedure TOpaRVCLDataModule.SetResultResource(aResult: TExternalEvaluatorResult);
var
...
function TOpaRVCLDataModule.GetS4Results(const aInput: TExternalEvaluatorinput;
  aResultData: TExternalEvaluatorResult): IS4Object;
var
  fileName : String;
  fn1: IRFunction;
...
begin
  ...
  begin
    ...
    try
      FEngine.SetSymbol('mat', FEngine.CreateNumericMatrix(aInput.InputMatrix)
      ...
      expr := fn1.Invoke(FEngine.CreateNumericMatrix(aInput.InputMatrix) as
        ISymbolicExpression);
      result := expr.AsS4;
...
procedure TOpaRVCLDataModule.ShowAboutForm(Sender: TObject);
begin
...
procedure TOpaRVCLDataModule.ExitDNSEABridgeWithWarning(Sender: TObject);
begin
  if MessageDlg(WARNING_CLOSE_SERVER, mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    Application.MainForm.Close;
end;

function TOpaRVCLDataModule.ValidateS4Results(const aNumNewParams: integer; aNames:
  TArray<string>; const aNumElements: integer): boolean;
begin
  result := (aNumNewParams = length(aNames));
end;

procedure TOpaRVCLDataModule.SetErrorMessage(const aErrorMessage: string; aResult:
  TExternalEvaluatorResult);
begin
